...
gulp.task('publish-samples', function (done) {
    var mapping = { 'ng': 'angular', 'react': 'react', 'javascript': 'javascript', 'ts': 'typescript', 'angular': 'angular' };

    var isMaster = common.isMasterBranch;
    var isHotFix = process.env.BRANCH_NAME.indexOf('hotfix-') !== -1;

    var demoPath = isMaster ? './production/demos' : './development/demos';
    demoPath = isHotFix ? './hotfix/' + process.env.BRANCH_NAME.split('hotfix-')[1] + '/demos' : demoPath;
    ...
    if (config.isShowCase) {
        ...
        demoPath = isHotFix ? `./hotfix/${process.env.BRANCH_NAME.split('hotfix-')[1]}/${platformName}` : demoPath;
    }
    else {
        for (var i = 0; i < platforms.length; i++) {
            if (common.currentPackage === `ej2-${platforms[i]}-samples`) {
                ...
                demoPath = isHotFix ? `./hotfix/${process.env.BRANCH_NAME.split('hotfix-')[1]}/${platformName}/demos` : demoPath;
            }
        }
    }
