...
export namespace Input {
    ...
    */
    ...
    function wireClearBtnEvents(element: HTMLInputElement, button: HTMLElement, container: HTMLElement): void {
      button.addEventListener('click', (event: MouseEvent) => {
        let state: boolean = element.classList.contains(CLASSNAMES.DISABLE) || element.readOnly ||
                             container.classList.contains(CLASSNAMES.DISABLE);
        if (!(state || (button.classList.contains(CLASSNAMES.CLEARICONHIDE) && !container.classList.contains('e-static-clear') ))) {
          event.preventDefault();
          if (element !== document.activeElement) { element.focus(); }
          element.value = '';
          addClass([button], CLASSNAMES.CLEARICONHIDE);
        } else if (button.classList.contains(CLASSNAMES.CLEARICONHIDE) && !state) {
    ...
    */
    ...
    export function setValue(value: string, element: HTMLInputElement, floatLabelType ?: string, clearButton?: boolean): void {
        ...
        if (!isNullOrUndefined(clearButton) && clearButton && !(<HTMLElement> element.nextSibling).classList.contains('e-clear-icon')) {
            let parentElement: HTMLElement = <HTMLElement> getParentNode(element);
            element.parentNode.insertBefore(createClearButton(element, parentElement), element.nextSibling);
            let button: HTMLElement = <HTMLElement> parentElement.getElementsByClassName(CLASSNAMES.CLEARICON)[0];
            if (element.value && parentElement.classList.contains('e-input-focus')) {
                removeClass([button], CLASSNAMES.CLEARICONHIDE);
            }
            if (element.value && !(<HTMLElement> element.nextSibling).classList.contains('e-clear-icon')) {
                 removeClass([button], CLASSNAMES.CLEARICONHIDE);
            } else {
                addClass([button], CLASSNAMES.CLEARICONHIDE);
            }
